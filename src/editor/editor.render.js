(function (w) {
    ...
    return doc.map(function (node) {
      ...
      switch (node.type) {
        ...
        case 'todo':
          ...
          text = esc(node.children && node.children[0] ? node.children[0].text : '');
          return '<div data-blockid="' + node.id + '" data-type="todo"><span class="todo-checkbox' + (checked ? ' checked' : '') + '"></span><span>' + text + '</span></div>';
        case 'attachment':
        case 'link-card':
        case 'musicCard':
  ...
  function parseEditorDOM(containerEl) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (type === 'todo') {
        var cb = el.querySelector('.todo-checkbox'), checked = cb ? cb.classList.contains('checked') : false;
